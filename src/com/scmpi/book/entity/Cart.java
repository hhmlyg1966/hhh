...
public class Cart {  
	 ...
	 * @return void
	 */
	public void addItem(OrderItem oi,User u){
		
		Integer pi=oi.getP().getPid();
		if(map.containsKey(pi)){
			OrderItem oim=map.get(pi);
			float discount=u.getDiscount()/100;
			oim.setProduct_id(oi.getProduct_id());
//			oim.setNumber(oim.getNumber()+oi.getNumber());
			oim.setOrder_num(oim.getOrder_num()+oi.getOrder_num());	//添加商品
			oim.setOrder_subtotal((oim.getOrder_num()*discount*oi.getP().getPrice())); //设置小计价格
		}else{
			map.put(pi, oi);
		}
	}
